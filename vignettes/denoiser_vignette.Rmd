...
Within the `denoiser` package, we specify two sets of movement and measurement equations that are used to filter the data, both being separate versions of the _constant velocity_ model, which assumes that the subject is moving at constant velocity, meaning changes in acceleration are taken as part of the measurement error. This model was used in our previous project and has been found to be (somewhat) effective at handling the measurement error observed in our experiments, which is why it is included here. 

In this vignette, I will focus mostly on the definition of the parameters rather than the reasoning behind the model. For this, I refer the interested reader to the explanation of the [constant_velocity](https://ndpvh.github.io/denoiser/reference/constant_velocity.html) or [directional_constant_velocity](https://ndpvh.github.io/denoiser/reference/directional_constant_velocity.html) functions instead.
...
\begin{equation}
  ...
  \end{bmatrix}_i = \begin{bmatrix}
    x \\
    y
  \end{bmatrix}_{i - 1} + v \begin{bmatrix}
    cos(\alpha)^2 \\
    sin(\alpha)^2
  \end{bmatrix} \Delta t
\end{equation}
where $v$ represents the constant speed at which a subject moves in a particular direction $\alpha$, and where $\Delta t$ represents the time interval between observations. The speed in each of the two dimensions x and y is weighted according to the direction in which they go, with a maximal speed in the y-direction when a person moves at 90 or 270 degrees and with a maximal speed in the x-direction when a person moves at 0 or 180 degrees. Under this specification, we have to define the latent state $\bm{x}$ as a three-dimensional vector, taking into account both position and speed so that:
...
For the movement equation, we then have to define the three-dimensional matrix $F$ and $W$, both of which depend on the direction $\alpha$ and the time interval $\Delta t$, so that:
...
\begin{equation}
  W_i = \begin{bmatrix}
    cos(\alpha)^4 \Delta t^2 \sigma_{v}^2 & 0 & cos(\alpha)^2 \Delta t \sigma_{v}^2  \\
    0 & sin(\alpha)^4 \Delta t^2 \sigma_{v}^2 & sin(\alpha)^2 \Delta t \sigma_{v}^2 \\
    cos(\alpha)^2 \Delta t \sigma_{v}^2 & sin(\alpha)^2 \Delta t \sigma_{v}^2 & \sigma_{v}^2
  \end{bmatrix}_i
\end{equation}
Notice that if you compute $F_i \bm{x}_{i - 1}$, you will get back the original equation for $x$ and $y$, showing that these parameters do indeed conform to our assume movement equation. Furthermore note that $W_i$ can be obtained by assuming some error on the speed $v$ and, similarly, working this out according to the movement equation denoted above.
...
In its current specification, there are still several parameters that need to be specified by the user. This includes the initial conditions $\bm{x}_0$ and $P_0$, the measurement variance $\sigma_R^2$, and the movement variance $\sigma_{v}^2$.

In `denoiser`, we specify broad though data-driven priors. Specifically, we defined the initial condition $\bm{x}_0$ as the vector of mean x- and y-positions alongside mean speed. The prior covariance matrix $P_0$ is defined as the diagonal matrix of observed variances for these same variables in $\bm{x}_0$. These broad priors were defined for practical utility, but may not be fit for each use-case. 

For the variances, we require users to specify the assumed error variance $\sigma_R^2$ through the `error` argument (see later). Based on the provided error variance and the observed variances in the data, we then specify the speed variance to be:
...
Another variant of the constant velocity model operates under the assumption that the latent position changes in the following way:
...
For the movement equation, we then define the four-dimensional matrix $F$ and $W$ so that:
...
For the measurement equation, we have to define the measurement matrix $H$, again assuming that we only measured positions without any indication of speeds, so that:
...
For the parameters that still need to be defined -- namely, the initial conditions $\bm{x}_0$ and $P_0$, the measurement variance $\sigma_R^2$, and the movement variances $\sigma_{v_x}^2$ and $\sigma_{v_y}^2$ -- we specify broad data-driven priors in a similar way to the _constant velocity model_ and compute the speed-related variances in the same way as before, yet for each dimension separately.
